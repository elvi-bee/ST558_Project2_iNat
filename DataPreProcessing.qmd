...
# parse timestamps
dt_utc <- suppressWarnings(ymd_hms(raw$time_observed_at, quiet = TRUE, tz = "UTC"))
dt_local <- with_tz(dt_utc, tzone = "America/New_York")

#  build  fields
summary_df <- summary_df |>
  mutate(
    # If dt_local is NA (missing time), still keep observed_on for month/year
    observed_hour  = hour(dt_local),
    observed_month = coalesce(month(dt_local), month(ymd(observed_on, quiet = TRUE))),
    observed_year  = coalesce(year(dt_local),  year(ymd(observed_on, quiet = TRUE)))
  ) |>
  mutate(
    observed_hour  = ifelse(!is.na(observed_hour)  & observed_hour  >= 0 & observed_hour  <= 23, observed_hour,  NA),
    observed_month = ifelse(!is.na(observed_month) & observed_month >= 1 & observed_month <= 12, observed_month, NA),
    observed_year  = as.integer(observed_year)
  )

# 4) Time-of-day bins (fixed clock, local time; ordered for nice legends)
day_levels <- c("Night","Dawn","Morning","Afternoon","Dusk")
summary_df <- summary_df |>
  mutate(
    day_period = case_when(
      !is.na(observed_hour) & (observed_hour >= 21 | observed_hour < 5)  ~ "Night",
      !is.na(observed_hour) &  observed_hour >= 5  & observed_hour < 7   ~ "Dawn",
      !is.na(observed_hour) &  observed_hour >= 7  & observed_hour < 12  ~ "Morning",
      !is.na(observed_hour) &  observed_hour >= 12 & observed_hour < 17  ~ "Afternoon",
      !is.na(observed_hour) &  observed_hour >= 17 & observed_hour < 21  ~ "Dusk",
      TRUE ~ NA_character_
    ),
    day_period = factor(day_period, levels = day_levels, ordered = TRUE)
  )


# process other temporal variables
summary_df <- summary_df |>
  mutate(
...
summary_df <- summary_df |>
  mutate(
    ...
    observed_hour   = as.integer(observed_hour),
    observed_month  = as.integer(observed_month)
  )
...
df_obsyr <- summary_df |>
  ...
  summarise(
    total_obs = n(),
    mean_per_species = mean(sum_sp, na.rm = TRUE),
    sd_per_species = sd(sum_sp, na.rm = TRUE)
  )
view(df_obsyr)
```
...
ggplot(scatter_df, aes(x = observations, y = unique_species, label = NAME)) +
